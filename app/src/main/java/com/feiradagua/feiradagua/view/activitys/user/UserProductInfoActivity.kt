...
import com.feiradagua.feiradagua.utils.specialMessageProductWeight
...
class UserProductInfoActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if(tutorial) {
            ...
            when(getOrigin) {
                1 -> {
                    setUpProductInfosFromProducer(getProductInfos)
                }
                2 -> {
                    setUpProductInfosFromUser(getProductInfos)
                }
                3 -> {
    ...
    private fun setUpProductInfosFromCart(cart: Cart?) {
        cart?.let {
            ...
            binding.tvProductWeightValue.text = cart.weight.specialMessageProductWeight()
        ...
        binding.btAddToCart.setOnClickListener {
            getProductInfos?.let { product ->
                val cart = Cart(product.id, product.name, product.description, product.price,
                        count * product.price, product.producerId, product.weight, product.photo)
    ...
    private fun setUpProductInfosFromProducer(product: Products?) {
        ...
        constraintSet.connect(binding.tvProductValue.id, ConstraintSet.BOTTOM, binding.parentView.id, ConstraintSet.BOTTOM, 20)
        constraintSet.connect(binding.tvProductValue.id, ConstraintSet.START, binding.tvProductNameTitle.id, ConstraintSet.START, 0)
        constraintSet.connect(binding.tvProductValue.id, ConstraintSet.TOP, binding.tvProductWeightValue.id, ConstraintSet.BOTTOM, 0)
        ...
        binding.tvProductWeightValue.text = product?.weight?.specialMessageProductWeight()
    ...
    private fun initTutorial() {
        ...
        tutorialInfos.forEach { item ->
            ...
            binding.tvProductWeightValue.text = item.weight.specialMessageProductWeight()
        }
